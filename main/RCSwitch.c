...
void enableReceiveInternal(RCSWITCH_t * RCSwitch) {
	//uint64_t gpio_pin_sel = (1ULL<<22);
	uint64_t gpio_pin_sel = (1ULL<<RCSwitch->nReceiverInterrupt);
	ESP_LOGI(TAG, "gpio_pin_sel=%llu", gpio_pin_sel);
	...
	//gpio_isr_handler_add(GPIO_NUM_22, handleInterrupt, RCSwitch);  // GPIO_NUM_22 DATA PIN!
	gpio_isr_handler_add(RCSwitch->nReceiverInterrupt, handleInterrupt, RCSwitch);  // nReceiverInterrupt is GPIO
}
